...
static int varint_decoder(lua_State *L)
{
    ...
    {
        lua_pushnumber(L, (lua_Integer)unpack_varint(buffer, len));
...
static int signed_varint_decoder(lua_State *L)
{
    ...
    {
        lua_pushnumber(L, (lua_Integer)(int64_t)unpack_varint(buffer, len));
        lua_pushinteger(L, len + pos);
    }
    return 2;
}
...
static int struct_unpack(lua_State *L)
{
    ...
    case 'i':
    {
        lua_pushnumber(L, __ld32(unpack_fixed32(buffer, out)));
        break;
    }
    case 'q':
    {
        // lua_pushnumber(L, (lua_Integer) * (int64_t *)unpack_fixed64(buffer, out));
        char temp[64];
    ...
    case 'f':
    {
        lua_pushnumber(L, (lua_Integer)__ldf(unpack_fixed32(buffer, out)));
        break;
    }
    case 'd':
    {
        lua_pushnumber(L, (lua_Integer)__ldd(unpack_fixed64(buffer, out)));
        break;
    }
    case 'I':
    {
    ...
    case 'Q':
    {
        // lua_pushnumber(L, (lua_Integer) * (uint64_t *)unpack_fixed64(buffer, out));
        char temp[64];
...
LUALIB_API int luaopen_protobuf_pb (lua_State *L)
{
    luaL_newmetatable(L, IOSTRING_META);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
